...
namespace TimSort4Net
{
	#region class ArrayTimSort<T>

	/// <summary>TimSort implementation for Array.</summary>
	/// <typeparam name="T">Type of item.</typeparam>
	internal class ArrayTimSort<T>
	{
		/// <summary>
		...
		private ArrayTimSort(T[] array, Comparison<T> comparer)
		{
			m_Array = array;
			m_Comparer = comparer;
		...
		/// <param name="comparer">The comparator to determine the order of the sort.</param>
		public static void Sort(T[] a, int lo, int hi, Comparison<T> c)
		{
			...
			var sorter = new ArrayTimSort<T>(a, c);
		...
		private static void binarySort(T[] a, int lo, int hi, int start, Comparison<T> c)
		{
		...
		private static int CountRunAndMakeAscending(T[] a, int lo, int hi, Comparison<T> c)
		{
			Debug.Assert(lo < hi);
			int runHi = lo + 1;
		...
		private static void CopyRange(T[] src, int srcIndex, T[] dst, int dstIndex, int length)
		{
		...
		private static int GallopLeft(T key, T[] array, int lo, int length, int hint, Comparison<T> comparer)
		{
		...
		private static int GallopRight(T key, T[] array, int lo, int length, int hint, Comparison<T> comparer)
		{
			Debug.Assert(length > 0 && hint >= 0 && hint < length);
		...
		private T[] EnsureCapacity(int minCapacity)
		{
			if (m_MergeBuffer.Length < minCapacity)
			{
				...
				if (newSize < 0) // Not bloody likely!
					...
					newSize = Math.Min(newSize, m_Array.Length >> 1);
